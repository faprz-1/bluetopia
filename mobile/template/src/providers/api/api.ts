...
export class ApiProvider {
  ...
  public mediaBaseURL: string = "http://localhost:3000/api/"
  ...
  private conditionalRetry(error: Observable<any>): Observable<any> {
    return error.flatMap((error: any)=>{
      ...
      (error.status >= 500 && error.status <= 503) || error.status == 505){
          return Observable.throw({error: 'No retry on: '+error.status})
        }
   ...
   * @param useToken boolean para  intentar usar token en la peticion
   */
